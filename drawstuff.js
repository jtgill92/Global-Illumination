...
function side(n, i, v1, v2) {
    var result = Vector.dot(n, Vector.cross( Vector.subtract(i, v1), Vector.subtract(v2, v1) ) );
...
function IsPointInsideTraingle(i, a, b, c, n)  {
    try {
    ...
    try {
        ...
        if (!(ray instanceof Array) || !(triangle instanceof Object))
            ...
            if(n_dot_dir == 0) {
               ...
               var exitsInside = IsPointInsideTraingle(isect, vertA, vertB, vertC, triNormal);
...
function shadeTriIsect(isect,triangle,lights, polygon) {
    try {
        if (   !(isect instanceof Object)|| !(triangle instanceof Array) || !(lights instanceof Array) || !(polygon instanceof Object))
            throw "shadeIsect: bad parameter passed";
        else  {
            // console.log(lights);
             // if not just rendering intersects
            var c = [0, 0, 0, 255]; // init the ellipsoid color to blac
            var ambient = polygon.material.ambient;
            var diffuse = polygon.material.diffuse;
            var specular = polygon.material.specular;
            // add light for each source
            // var lightOccluded = false; // if an occluder is found
            var Lloc = new Vector(-3, 1, -0.5);
            for (var l=0; l<lights.length; l++) {
                // var Lloc = new Vector(lights[l].x, lights[l].y, lights[l].z);
                // add in the ambient light
                c[0] += lights[l].ambient[0] * ambient[0]; // ambient term r
                c[1] += lights[l].ambient[1] * ambient[1]; // ambient term g
                c[2] += lights[l].ambient[2] * ambient[2]; // ambient term b
               ...
               // Lloc.set(lights[l].x,lights[l].y,lights[l].z);
                ...
                var vertB = new Vector(triangle[1][0], triangle[1][1], triangle[1][2]); 
                var vertC = new Vector(triangle[2][0], triangle[2][1], triangle[2][2]);     
                var N = Vector.normalize(Vector.cross( Vector.subtract(vertB, vertA), 
                                          Vector.subtract(vertC, vertA)));  
                //console.log(N);
                var diffFactor = Math.max(0,Vector.dot(N,L));
                if (diffFactor > 0) {
                    c[0] += lights[l].diffuse[0] * diffuse[0] * diffFactor;
                    c[1] += lights[l].diffuse[1] * diffuse[1] * diffFactor;
                    c[2] += lights[l].diffuse[2] * diffuse[2] * diffFactor;
                } // end nonzero diffuse factor
                ...
                if (specFactor > 0) {
                    var newSpecFactor = specFactor;
                    for (var e=1; e < polygon.material.n; e++) // mult by itself if needed
                        newSpecFactor *= specFactor;
                    c[0] += lights[l].specular[0] * specular[0] * newSpecFactor; // specular term
                    c[1] += lights[l].specular[1] * specular[1] * newSpecFactor; // specular term
                    c[2] += lights[l].specular[2] * specular[2] * newSpecFactor; // specular term                    
                } // end if light not occluded
            } // end for lights
            ...
            c[1] = 255 * Math.min(1,c[1]); // clamp max value to 1
            c[2] = 255 * Math.min(1,c[2]); // clamp max value to 1
            var color = new Color(c[0], c[1], c[2], 255);
            return(color);
        } // if not just rendering isect
    } // end throw
...
function rayCastEllipsoids(context) {
    ...
    /* New */
    var cornellBox = getJSONFile(CORNELL_BOX_URL, "cornellbox");
    /* /New */
...
const CORNELL_BOX_URL = 
    "https://jtgill.github.io/raytracer/cornellbox.json";
